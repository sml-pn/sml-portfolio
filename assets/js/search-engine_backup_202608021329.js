var SMLEngine = (function() {
  var docs = [];
  var isLoaded = false;
  var lastService = null; // Memória do último serviço mencionado
  ...
  function search(query) {
    ...
    // Detectar afirmações de interesse (sim, quero, etc.)
    var isAffirmative = /^(sim|quero|sim, quero|quero sim|sim quero|com certeza|claro|pode ser|ok|yes|yep)/i.test(query);
    
    // Se o usuário respondeu afirmativamente e temos um último serviço em mente
    if (isAffirmative && lastService) {
      for (var i = 0; i < docs.length; i++) {
        if (docs[i].id === lastService) {
          trackEvent('chat_context_continue', { intent: lastService });
          lastService = null; // Limpa o contexto após usar
          return docs[i].resposta;
        }
      }
    }

    // Prioridade máxima: "chat" relacionado a "chat rag" ou "chat para site"
    if (q.match(/chat/)) {
      if (q.match(/rag|bot|ia|inteligencia|assistente|automa|para site|para meu site|igual|quero chat/)) {
